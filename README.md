...
Use `latest.fme.geojson` for a stable FME schema with `null` properties retained but a smaller field set than `latest.geojson`. It removes unit fields, coordinate properties duplicated from geometry, `valid_to`, `parameter_profile`, `available_parameter_count`, and detailed `discharge_*` properties except `discharge_class`. The `has_discharge` capability flag is retained.
