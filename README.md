...
Both compact GeoJSON endpoints are pre-generated by `run-hourly` and served from files when available, so ArcGIS/VertiGIS requests do not need to rebuild the payload every time.
...
- `refresh-geojson-cache` rebuilds only the cached compact GeoJSON files.
