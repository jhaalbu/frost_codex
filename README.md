...
All three latest GeoJSON endpoints are pre-generated by `run-hourly` and served from files when available, so ArcGIS/VertiGIS/FME requests do not need to rebuild the payload every time. The cache also includes `.gz` files that are served automatically to clients that send `Accept-Encoding: gzip`. Filtered `latest.geojson?has=...` requests are still built from the database.
...
- `run-hourly` also refreshes cached `latest.geojson`, `latest.compact.geojson` and `latest.7d.geojson` files after the database commit.
...
- `FROST_GEOJSON_CACHE_DIR` controls where `latest.geojson`, `latest.compact.geojson` and `latest.7d.geojson` are pre-generated; the web app serves these files directly when they exist.
