...
Both compact GeoJSON endpoints are pre-generated by `run-hourly` and served from files when available, so ArcGIS/VertiGIS requests do not need to rebuild the payload every time. The cache also includes `.gz` files that are served automatically to clients that send `Accept-Encoding: gzip`.
...
- `run-hourly` also refreshes cached `latest.compact.geojson` and `latest.7d.geojson` files after the database commit.
...
- `refresh-geojson-cache` rebuilds only the cached compact GeoJSON files, including gzip-compressed `.gz` variants.
...
- `FROST_GEOJSON_CACHE_DIR` controls where `latest.compact.geojson` and `latest.7d.geojson` are pre-generated; the web app serves these files directly when they exist.
